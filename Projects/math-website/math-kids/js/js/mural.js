...
$(document).ready(function() {
    $('#mural').bind('contextmenu', function(e) {
        return false;
    }).on('dragstart', function(event) {
        event.preventDefault();
    });
    if (!/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
/*         loopClouds();  Turned off cause makes things too slow... :'( */
        mobile = 0;
    }
...
function resizeAnim(anima, bgProportion, numFrames, roundTF) {
    var toon = anima;
    if (roundTF == 1) var ratio = Math.round(toon.height() / toon.width() * 10) / 10;
    else var ratio = Math.round(toon.height() / toon.width());
